...
const Dashboard: React.FC<DashboardProps> = ({ records, circles, onDeleteRecord, onEditRecord, onNavigate, themeId = 'default' }) => {
  ...
  const headerClass = isCustomTheme
    ...
    : 'bg-mahjong-800/95 backdrop-blur-md shadow-lg';
  ...
  return (
    <div className={`flex flex-col h-full ${isCustomTheme ? 'bg-transparent' : 'bg-white/30'}`}>
      {/* Header / Summary Card */}
      <div className={`${headerClass} p-6 pt-10 rounded-b-[2.5rem] flex-shrink-0 z-10 relative transition-all duration-300`}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        ...
        {filteredRecords.length === 0 ? (
          ...
          filteredRecords.map((record) => (
            <div 
              key={record.id} 
              className={`group backdrop-blur-md rounded-xl p-4 shadow-sm relative overflow-hidden transition-all active:scale-[0.99] ${
                  isCustomTheme 
