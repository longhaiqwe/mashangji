...
const ThemeSettings: React.FC<ThemeSettingsProps> = ({ preferences, onUpdatePreferences, onNavigate }) => {
  ...
  const themes = [
    ...
    { id: 'green', name: '麻将绿', color: 'bg-mahjong-800', preview: 'bg-gradient-to-br from-mahjong-800 to-mahjong-600' },
    { id: 'red', name: '喜庆红', color: 'bg-red-700', preview: 'bg-gradient-to-br from-red-600 to-orange-600' },
  ];
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {themes.map((theme) => (
            <button
              ...
              className={`relative aspect-[3/5] rounded-xl overflow-hidden shadow-sm border-2 transition-all group ${
                preferences.themeId === theme.id ? 'border-mahjong-600 ring-2 ring-mahjong-200' : 'border-transparent hover:border-gray-300'
              }`}
            >
          ...
          <button
            ...
            className={`relative aspect-[3/5] rounded-xl overflow-hidden shadow-sm border-2 transition-all flex flex-col items-center justify-center bg-gray-100 ${
              preferences.themeId === 'custom' ? 'border-mahjong-600 ring-2 ring-mahjong-200' : 'border-dashed border-gray-300 hover:border-gray-400'
            }`}
          >
